...
class Distributed():
    ...
    def multiprocess(self, map_keys=-1, chunk_size=1, sized=False, time_out=20, group_by=0, reduce_fn=None, stream=True, use_sid=False, stream_handler=None,
        ...
        """
        self.actual_workers = num_workers if num_workers is not None else self.num_workers
        if self.max_size != max_size and max_size is not None:
            self.max_size = max_size
            self.output_queue = Queue(maxsize=max_size)
        logger.info(f"actual_workers: {self.actual_workers}, output_queue size: {self.max_size}")
